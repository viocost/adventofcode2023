...
data = """rhqrpdxsqhgxzknr2foursnrcfthree
2bmckl
four95qvkvveight5
...
def get_real_last(word):
    i = len(word) - 1
    while i >=0:
        if is_digit(word[i]):
            return word[i]
        match = re.search(r'(one|two|three|four|five|six|seven|eight|nine)', word[i:])
        if match:
            return word_to_digit[match.group(1)]
        i -= 1
...
def replace_digits(word):
    pre = re.sub(r'(one|two|three|four|five|six|seven|eight|nine)', replacer, word)
    return re.sub(r'[a-z]*', '', pre) + get_real_last(word)
...
d = list(map(lambda word: replace_digits(word), ds.split('\n')))
