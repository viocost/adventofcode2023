import re
data = """Game 1: 3 blue, 7 green, 10 red; 4 green, 4 red; 1 green, 7 blue, 5 red; 8 blue, 10 red; 7 blue, 19 red, 1 green
...
def parse_game(gamestr) -> [Take]:
    entries = gamestr.split(', ')
    takes = []
    game = Take()
...
games = list(map(lambda line: re.sub(r'(Game.*\:\s+)', '', line).split('; '),  data.split('\n')))

res = 0

for i in range(len(games)):
    takes = list(map(parse_game, games[i]))
    possible = True
    for take in takes:
        if take.red > 12 or take.green > 13 or take.blue > 14:
            possible = False
...
for i in range(len(games)):
    ...
    minimal = Take()
    for take in takes:
        minimal.red = max(minimal.red, take.red)
        minimal.blue = max(minimal.blue, take.blue)
        minimal.green = max(minimal.green, take.green)
    res2 += minimal.red * minimal.blue * minimal.green
