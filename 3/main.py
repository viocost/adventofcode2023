...
data = """...............307............130..................969...601...186.........................................312....628..........878..........
......479#../..*..............................#.....*......*............../309.....484........................*......-..........+.....89....
...
def is_part_number(mtch: Match[str], i: int, lines: list)-> bool:
    line = lines[i]
    start, end = mtch.span()
    if start > 0 and is_symbol(line[start-1]):
        return True

    if end < len(line) - 1 and is_symbol(line[end]):
        return True

    if i > 0 and is_symbol(lines[i-1][max(start-1, 0):min(end+1, len(line)-1)]):
        return True


    if i < len(lines)-1 and is_symbol(lines[i+1][max(start-1, 0):min(end+1, len(line)-1)]):
        return True

    return False


def get_gear(mtch: Match[str], i: int, lines: list)->int:
    line = lines[i]
    start, end = mtch.span()
    adj_parts = []
    ...
    for mtch in matches:
        m_start, m_end = mtch.span()
        if m_end == start or m_start == end:
            adj_parts.append(int(mtch.group()))


    if i > 0:
        prev_line = lines[i-1]
        matches = re.finditer(r'[0-9]+', prev_line)
        for mtch in matches:
            m_start, m_end = mtch.span()
            if m_start - 1 <= start and start <= m_end:
                adj_parts.append(int(mtch.group()))

    if i < len(lines):
        next_line = lines[i+1]
        matches = re.finditer(r'[0-9]+', next_line)
        for mtch in matches:
            m_start, m_end = mtch.span()
            if m_start - 1 <= start and start <= m_end:
                adj_parts.append(int(mtch.group()))
...
def update_lines(lines):
    ...
    for i, line in enumerate(lines):
        new_line = line
        matches = re.finditer(r'[0-9]+', line)
        offset = 0  # Offset to account for changes in string length during replacements

        for mtch in matches:
            if not is_part_number(mtch, i, lines):
                start, end = mtch.span()
                start += offset
                end += offset
                match_length = end - start
                replacement = '.' * match_length  # Replacement string of dots with the same length as the match
                new_line = new_line[:start] + replacement + new_line[end:]
                offset += len(replacement) - match_length  # Update offset
...
#         s = ""
#         for mtch in matches:
#             s += mtch.group() + " " + str(mtch.span()) + "  "
#             if is_part_number(mtch, i, lines):
#                 res+=int(mtch.group())

#         print(s)

res = 0
updated_lines = update_lines(lines)
for i in range(len(updated_lines)):
    matches = re.finditer(r'\*{1}', updated_lines[i])
    if matches:
        for mtch in matches:
           r =get_gear(mtch, i, updated_lines)
           print(r)
           res +=r
